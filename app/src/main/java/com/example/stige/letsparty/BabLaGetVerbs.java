...
import com.stigeborn.Sms.ResponseAlgorithm;
 ...
 */
public class BabLaGetVerbs extends AsyncTask<String, Void, String> {
    ...
    BabLaGetVerbs(List<Sms> sms, HashMap<String, Integer> map) throws Exception{
        this.initSms = sms;
        this.initMap = map;
    }
        ...
        for ( String word : initMap.keySet() ) {
            try {
                ...
                while ((inputLine = in.readLine()) != null){
                    ...
                    if(inputLine.contains("{verb}") || inputLine.contains("{adjektiv}") || inputLine.contains("{substantiv}")){
                        getData = true;
                    }
                    ...
                    if(getData ){// !inputLine.contains("Inga synonymer hittades") && inputLine.length()>55){
                        ...
                        int start = inputLine.indexOf("class=\"result-link\">")+20; //+20 to remove class=...-link">
                        ...
                        * GER FEL NÄR DET NYA ORDET ÄR KORTARE ÄN DET TIDIGARE
                        ...
                        if(!word_in_base_form.contains("?") && !word_in_base_form.contains("\"") && !word_in_base_form.contains(".") && !word_in_base_form.equals(word)) {
                            ...
                            if (!returnMap.containsKey(word)) {
                                returnMap.put(word, initMap.get(word));
                            } else {
                                returnMap.put(word, returnMap.get(word) + initMap.get(word));
                            }
                        }
                    }else{
