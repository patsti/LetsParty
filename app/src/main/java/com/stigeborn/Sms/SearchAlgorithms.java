...
import com.example.stige.letsparty.Rellations;
...
public class SearchAlgorithms {
    public List<Conversation> findMatch(String sms, List<Sms> lSms){
        ...
        for(int i=0; i<matchRate; i++){
            smsContent[i].toLowerCase().replace("?","").replace("!", "").replace(".","").replace(",","");
        }
        ...
        for(int i = 1; i < lSms.size(); i++){
            ...
            if(lSms.get(i).getFolderName().contains("inbox")) {
                for(int j = 0; j < matchRate; j++){
                    String message = lSms.get(i).getMsg().toLowerCase().replace("?", "").replace("!","").replace(".","").replace(",","").toLowerCase();
